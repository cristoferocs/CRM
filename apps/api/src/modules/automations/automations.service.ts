...
import { executeNodeByType } from "./node-executors/index.js";
import { buildVariableBag, makeInterpolator } from "./node-executors/variables.js";
import { evaluateCondition as evalCond } from "./condition.evaluator.js";

// ---------------------------------------------------------------------------
// Trigger-filter helper (legacy schema kept for backward compat)
// ---------------------------------------------------------------------------
    ...
    return evalCond(
        {
            ...
            operator: condition.operator as Parameters<typeof evalCond>[0]["operator"],
            value: condition.value,
        } as Parameters<typeof evalCond>[0],
        { contact: { id: "", customFields: data }, deal: { id: "", customFields: data } } as never,
    );
}
...
export class AutomationsService {
    ...
    async trigger(event: AutomationTriggerEnum, payload: Record<string, unknown>, orgId: string): Promise<void> {
        ...
        for (const automation of automations) {
            ...
            if (conditions.length > 0) {
                const andResults = conditions.filter(c => c.logic !== "OR").map(c => evaluateTriggerCondition(c, payload));
    ...
    async execute(automationId: string, triggerData: Record<string, unknown>, orgId: string, isDryRun = false): Promise<NodeExecutionResult[]> {
        ...
        const conversationId = triggerData.conversationId as string | undefined;

        const [contact, deal, org] = await Promise.all([
        ...
        // Resolve owner (deal.ownerId preferred, otherwise org's first active user)
        let owner: { name: string | null; email: string | null } | null = null;
        if (deal?.ownerId) {
            owner = await prisma.user.findUnique({ where: { id: deal.ownerId }, select: { name: true, email: true } });
        }

        const previousOutputs = new Map<string, unknown>();
        const variables = buildVariableBag({
            contact, deal: deal as never, owner, org,
            triggerData, triggerType: automation.triggerType,
            previousOutputs,
        });
        const interpolate = makeInterpolator(variables);
        ...
        const ctx = {
            ...
            isDryRun,
            contact, deal: deal as never,
            previousOutputs, variables, interpolate,
        };
        ...
        while (queue.length > 0) {
            ...
            const partial = await executeNodeByType(node, ctx);
            ...
            previousOutputs.set(node.id, result.output);
            // Refresh prev namespace so subsequent nodes can read previous outputs.
            (variables as Record<string, unknown>).prev = Object.fromEntries(previousOutputs);
            results.push(result);
            const outgoing = edges.filter(e => e.from === node.id);
            for (const edge of outgoing) {
                if (node.type === "condition" || node.type === "ab_test") {
